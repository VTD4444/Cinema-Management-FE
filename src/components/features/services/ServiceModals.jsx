...
const ServiceModals = ({ state, onClose }) => {
    ...
    if (isDelete) {
        return (
            <Modal isOpen={true} onClose={onClose} className="max-w-md bg-[#161616] border-border/50 relative">
                <button
                    onClick={onClose}
                ...
                <div className="p-2 space-y-6 flex flex-col items-center justify-center text-center mt-2">
                    ...
                    <div className="space-y-2">
                        ...
                        <p className="text-sm text-zinc-400 max-w-[280px] mx-auto leading-relaxed">
                            Hành động này không thể hoàn tác. Bạn có chắc chắn muốn xóa "<span className="text-zinc-200 font-medium">{data?.name}</span>" khỏi hệ thống?
                        </p>
                    </div>

                    <div className="flex w-full gap-3 pt-2">
                        <Button variant="secondary" onClick={onClose} className="flex-1 rounded-full bg-zinc-800/80 border-zinc-700/50 hover:bg-zinc-700 text-gray-300">
                            Hủy
                        </Button>
                        <Button variant="danger" className="flex-1 rounded-full shadow-lg shadow-red-500/20 hover:shadow-red-500/40 transition-shadow">
                            Xác nhận xóa
                        </Button>
                    </div>
                </div>
    ...
    if (isAddOrEdit) {
        return (
            <Modal isOpen={true} onClose={onClose} title={state.type === 'add' ? 'Thêm Dịch vụ mới' : 'Thêm / Chỉnh sửa Dịch vụ'} className="max-w-3xl bg-[#161616] border-border/50">
                <form className="space-y-6 animate-in fade-in zoom-in-95 duration-200" onSubmit={(e) => e.preventDefault()}>
                    <p className="text-sm text-zinc-500 -mt-2 mb-6">Vui lòng điền đầy đủ thông tin dịch vụ dưới đây</p>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div className="col-span-1 space-y-3">
                            ...
                            <div className="p-3 bg-red-950/20 border border-red-900/30 rounded-xl mt-4">
                                <p className="text-xs text-red-400/80 leading-relaxed italic">
                                    * Lưu ý: Hình ảnh nên có tỉ lệ 1:1 và độ phân giải tối thiểu 500x500px để hiển thị tốt nhất.
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="col-span-2 space-y-5">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <Select label="Loại" defaultValue={data?.type || "Đồ ăn"} className="bg-zinc-900/50 border-zinc-800 rounded-xl h-11">
                            ...
                            <div className="relative">
                                ...
                                />
                                <span className="absolute right-4 top-[34px] text-sm text-zinc-500 pointer-events-none">VNĐ</span>
                            </div>
    ...
    if (isView) {
        return (
            <Modal isOpen={true} onClose={onClose} className="max-w-4xl p-0 bg-[#161616] border-border/50 overflow-hidden shadow-2xl">
                <div className="flex flex-col md:flex-row h-full">
                    {/* Left Image Area (Large) */}
                    <div className="w-full md:w-[45%] bg-zinc-900 relative">
                        {data?.image ? (
                            <img src={data.image} alt={data.name} className="w-full h-full object-cover min-h-[400px]" />
                        ) : (
                    ...
                    <div className="w-full md:w-[55%] p-8 flex flex-col relative h-[500px]">
                        <div className="flex items-center gap-2 mb-3">
                            <span className="text-[10px] uppercase font-bold tracking-wider text-amber-500 bg-amber-500/10 px-2 py-0.5 rounded border border-amber-500/20">{data?.type}</span>
                        ...
                        <div className="flex items-center gap-12 border-b border-border/40 pb-6 mb-6">
                            <div>
                                <p className="text-[10px] text-zinc-500 font-medium uppercase tracking-wider mb-1">GIÁ NIÊM YẾT</p>
                                <p className="text-2xl font-bold text-red-500">{data?.price.toLocaleString('vi-VN')} VNĐ</p>
                            </div>
                            <div>
                                <p className="text-[10px] text-zinc-500 font-medium uppercase tracking-wider mb-1">SỐ LƯỢNG TỒN KHO</p>
                                <div className="flex items-center gap-2">
                                    <p className="text-xl font-bold text-white">{data?.stock}</p>
                                    <span className="text-[10px] text-emerald-500 bg-emerald-500/10 px-1.5 py-0.5 rounded-full border border-emerald-500/20 whitespace-nowrap">Ổn định</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="pt-6 mt-4 flex items-center justify-between">
                            <div className="flex flex-col">
                                <span className="text-[10px] text-zinc-500 uppercase font-medium">Trạng thái kinh doanh</span>
                            ...
                            <div className="flex gap-3">
                                <Button variant="secondary" className="rounded-full px-5 bg-zinc-800/80 hover:bg-zinc-700 border-zinc-700/50 gap-2 h-10 shadow-sm">
                                    <Pencil className="h-3.5 w-3.5" />
                                    Chỉnh sửa
                                </Button>
                                <Button variant="danger" onClick={onClose} className="rounded-full px-6 shadow-lg shadow-red-500/20 hover:shadow-red-500/40 h-10 gap-2">
                                    Đóng chi tiết
                                </Button>
                            </div>
                        </div>
