...
const Cities = () => {
  ...
  const onModalSuccess = () => fetchCities();
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-bold tracking-tight text-white">Thành phố/Tỉnh</h2>
        <Button
      ...
      <div className="rounded-xl border border-border bg-surface/30 p-1 flex flex-col gap-1 overflow-hidden shadow-sm">
          ...
          <Table className="border-0 rounded-none bg-transparent">
            ...
            <TableBody>
              {loading ? (
                ...
                <TableRow className="border-b border-border/20">
                  <TableCell colSpan={3} className="py-12 text-center text-zinc-500">Chưa có dữ liệu.</TableCell>
                </TableRow>
              ) : (
                cities.map((city) => (
                  <TableRow key={city.id} className="border-b border-border/20 hover:bg-white/5 transition-colors group">
                      ...
                      <div>
                        ...
                        {typeof city.cinema_complex_count === 'number' && (
                          <p className="text-xs text-zinc-500 mt-1 flex items-center gap-1">
                            ...
                            {city.cinema_complex_count} cụm rạp
                          </p>
                        )}
                      </div>
                    </TableCell>
                    <TableCell className="pr-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-1 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity duration-200">
                        <button
                          onClick={() => setModalState({ type: 'edit', data: city })}
                        ...
                        <button
                          onClick={() => setModalState({ type: 'delete', data: city })}
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-border/50 bg-surface/50 rounded-b-lg">
          <span className="text-sm text-zinc-500">
            ...
            <span className="font-semibold text-zinc-300">{total}</span> tỉnh/thành
          </span>
      ...
      <CityModals state={modalState} onClose={closeModal} onSuccess={onModalSuccess} />
    </div>
  );
};
