...
const SeatSelection = () => {
  ...
  useEffect(() => {
    ...
    newSocket.on('seatLocked', ({ seatId, userId }) => {
      // Update local seatMap if someone else locked a seat
      if (userId !== user?.id) {
         setSeatMap(prev => {
            const next = { ...prev };
            Object.keys(next).forEach(row => {
               next[row] = next[row].map(s => String(s.id) === String(seatId) ? { ...s, status: 'HOLDING' } : s);
            });
            return next;
         });
      }
    ...
    newSocket.on('error', (msg) => {
        alert(msg); // simple alert for socket errors
        // You might want to rollback the selected state if the lock failed
    });
                      ...
                      {seatMap[rowLabel].map((seat) => {
                        ...
                        return (
                          <button
                            ...
                            className={`h-9 rounded flex items-center justify-center text-xs font-bold transition-all ${
                              seat.type === 'COUPLE' ? 'w-16' : 'w-9'
            ...
            <div className="sticky top-28 bg-[#141414] border border-zinc-800 rounded-2xl overflow-hidden shadow-2xl">
              ...
              <div className="p-6 space-y-4 text-sm">
                 <div className="flex justify-between border-b border-dashed border-zinc-800 pb-4">
                   <span className="text-zinc-400">Ngày / Giờ</span>
